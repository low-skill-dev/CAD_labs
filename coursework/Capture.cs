...
public static class Capture
{
	...
	public static bool isValidPoint(PointF target, SizeF Rect)
	{
		...
		if(target.X > Rect.Width - 1) {
			return false;
		}

		if(target.Y > Rect.Height - 1) {
			return false;
		}
	...
	public static bool IsCaptured(RectangleF captureRect, ArcF arc, bool partialCaptureMode = false)
	{
		...
		if(IsInBounds(captureRect, curr)) {
			if(partialCaptureMode) {
				return true; // точка попала и включен режим попадания части
			}
		} else {
			if(!partialCaptureMode) {
				return false; // точка не попала и включен режим частичного попадания
			}
		}
		...
		if(dir < 0) {
			...
			for(var a = closestAxisAngle; a > endA; a -= PI / 2) {
				curr = Common.FindPointOnCircle(arc.Center, arc.Radius, endA);
				if(IsInBounds(captureRect, curr)) {
					if(partialCaptureMode) {
						return true; // точка попала и включен режим попадания части
	...
	internal static bool IsCaptured(RectangleF captureRect, CircleF circle, bool partialCaptureMode = false)
	{
		if(!partialCaptureMode) {
			PointF curr;
			for(var mx = -1; mx <= 1; mx += 1) { // m = multiplier, -1 -> 0 -> 1
				for(var my = -1; my <= 1; my += 1) {
	...
	{
		if(useSimpleAlg) {
			return IsInBounds(captureRect, filler.StartPoint);
		}
	...
	internal static bool IsCaptured(RectangleF captureRect, LineF line, bool partialCaptureMode = false)
	{
		...
		if((startCapt || endCapt) && partialCaptureMode) {
			return true;
		}
